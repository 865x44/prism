...
2. **Develop the model** into `developed_model`, with these fields:
   ...
   - `synthesis` presentation invariant: a synthesis longer than 2000 characters MUST contain at least one blank line (paragraph break). Long single-line walls are rejected by the checkpoint. This is presentation only — split the same prose into paragraphs; never change claims, reasoning, or structure to satisfy it.
