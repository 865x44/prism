...
- **Internal Search policies**: `initial` (broad structural search), `residual` (novelty against accumulated field), `rift` (explicit in manual use; mandatory second Search policy inside AUTO; third Search pass inside PACK and BONK).
- **Superseded / deprecated terms**: `/pizm 360` is a deprecated user-facing alias to `Search(residual)`, while the `pizm-candidates-v1` wire value `"360"` is the current compatibility encoding of the residual search policy and stays accepted until a `RESIDUAL` wire value ships (the alias may go; the encoding does not); `/pizm forge` is retained for one release solely as a deprecated compatibility alias to BONK; "Breadth" is superseded as a user mode (Search is the manual divergence primitive); "MAX" is superseded and eliminated as a product route; raw-P-only AUTO is superseded by Portfolio target nomination (P or B); compact-card Deep is superseded by mature analytical prose synthesis (~900–1600 words for P, ~1400–2400 words for B); full rubric-blindness is operationalized as same-host staged contract separation post-freeze.
