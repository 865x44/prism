"""Explore entrypoint for Perspective Core v0.

Implements NORMAL and 360 modes: two-stage provider orchestration
with bounded schema repair, KEEP-only P-ID registration,
deterministic territory reconstruction, and PassRecord persistence.
"""
    ...
    if "```" in text:
        ...
        for block in blocks[1::2]:
            ...
            if lines and lines[0].strip().startswith("{"):
                continue
            candidate = "\n".join(lines[1:] if lines and not lines[0].strip().startswith("{") else lines)
            try:
...
# ─────────────────────────────────────────────────────────────────────────────
# Candidate parsing from Call A response
# ─────────────────────────────────────────────────────────────────────────────
...
def _parse_generate_response(
    ...
    """Parse Call A NORMAL response into Diagnosis and candidates.
    ...
    for idx, c_data in enumerate(candidates_data):
        ...
        candidate = PerspectiveCandidate(
            ...
            operator_ids=c_data.get("operator_ids", []),
...
def _parse_360_generate_response(
    raw_text: str, candidate_budget: int
) -> tuple[PriorSummary, Diagnosis, list[PerspectiveCandidate]]:
    """Parse 360 Call A response into PriorSummary, Diagnosis, and candidates.
    ...
    # Parse prior_summary
    prior_data = data.get("prior_summary")
    if not prior_data or not isinstance(prior_data, dict):
        raise ValueError("Missing prior_summary in 360 generate response")

    prior_summary = PriorSummary.from_dict(prior_data)
    ...
    return prior_summary, diagnosis, candidates
...
def _invoke_stage_with_repair(
    ...
    *,
    ...
    Args:
        ...
        parent_stage: Stage name for the primary call
        prompt: Primary prompt
        repair_prompt: Repair prompt template (<<ERROR>> is substituted with the parse error)
        parse_fn: Function to parse response (raises ValueError on failure)
    ...
    Raises:
        ValueError: If both primary and repair fail
    """
    raw_text = _invoke_stage(
        provider, trace_writer, stage=parent_stage, prompt=prompt, invocation_id=invocation_id
    )
...
def run_explore(
    ...
    *,
    ...
    """Run NORMAL or 360 exploration.

    Signature is frozen by Wave 1 execution contract.
    """
    if request.mode not in ("normal", "360"):
        raise ValueError(f"explore.run_explore does not support mode: {request.mode}")
    ...
    if session_store.exists(session_id):
        ...
        if request.constraint_ledger.entries:
            session.constraint_ledger = ConstraintLedger.from_dict(request.constraint_ledger.to_dict())
    ...
    trace_writer.write_json("request.json", {
    ...
    # ── Branch by mode: NORMAL vs 360 ───────────────────────────────────────
    if request.mode == "normal":
        generate_prompt_template = prompt_path("explore_normal_generate.md").read_text(encoding="utf-8")
        select_prompt_template = prompt_path("explore_normal_select.md").read_text(encoding="utf-8")
        repair_generate_template = prompt_path("explore_normal_repair_generate.md").read_text(encoding="utf-8")
        repair_select_template = prompt_path("explore_normal_repair_select.md").read_text(encoding="utf-8")

        generate_stage = "EXPLORE_GENERATE"
        select_stage = "EXPLORE_SELECT"
        ...
        def parse_generate(text: str) -> tuple[Diagnosis, list[PerspectiveCandidate]]:
            return _parse_generate_response(text, request.candidate_budget)
        ...
        diagnosis, candidates = _invoke_stage_with_repair(
            ...
            repair_prompt=repair_generate_template.replace(
                "<<SOURCE>>", source
            ).replace("<<OBJECTIVE>>", session.objective),
        ...
        trace_writer.write_json("candidates.json", {
        ...
        prior_territory = build_prior_territory(session)

        generate_prompt_template = prompt_path("explore_360_generate.md").read_text(encoding="utf-8")
        select_prompt_template = prompt_path("explore_360_select.md").read_text(encoding="utf-8")
        repair_generate_template = prompt_path("explore_360_repair_generate.md").read_text(encoding="utf-8")
        repair_select_template = prompt_path("explore_360_repair_select.md").read_text(encoding="utf-8")

        generate_stage = "EXPLORE_360_GENERATE"
        select_stage = "EXPLORE_360_SELECT"
        ...
        generate_prompt = (
            ...
            .replace("<<MUST_NOT_CLAIM>>", _render_must_not_claim(request.must_not_claim))
            .replace("<<OPERATOR_HINTS>>", _render_operator_hints())
            .replace("<<ACTIVE_PERSPECTIVES>>", _render_existing_perspectives(session))
            .replace("<<EXPLORED_TERRITORY>>", render_explored_territory(prior_territory))
            .replace("<<CANDIDATE_BUDGET>>", str(request.candidate_budget))
        )

        def parse_360_generate(text: str) -> tuple[PriorSummary, Diagnosis, list[PerspectiveCandidate]]:
            return _parse_360_generate_response(text, request.candidate_budget)

        prior_summary, diagnosis, candidates = _invoke_stage_with_repair(
            provider,
            trace_writer,
            parent_stage=generate_stage,
            prompt=generate_prompt,
            ...
            parse_fn=parse_360_generate,
        )

        # Persist prior-summary, diagnosis, and candidates
        trace_writer.write_json("prior-summary.json", prior_summary.to_dict())
        trace_writer.write_json("diagnosis.json", diagnosis.to_dict())
        trace_writer.write_json("candidates.json", {
            "candidates": [c.to_dict() for c in candidates]
        })
        ...
        select_prompt = (
            select_prompt_template.replace("<<SOURCE>>", source)
            .replace("<<CONSTRAINTS>>", _render_constraints(session.constraint_ledger))
            .replace("<<PRIOR_SUMMARY>>", render_prior_summary(prior_summary))
            .replace("<<DIAGNOSIS>>", _render_diagnosis_for_selection(diagnosis))
            .replace("<<ACTIVE_PERSPECTIVES>>", _render_existing_perspectives(session))
            .replace("<<EXPLORED_TERRITORY>>", render_explored_territory(prior_territory))
            .replace("<<CANDIDATES>>", _render_candidates_for_selection(candidates))
    ...
    if not candidates:
        # No candidates → no territory found
        ...
        trace_writer.write_json("result.json", {"outcome": "NO_NEW_STRONG_TERRITORY_FOUND_THIS_PASS"})
    ...
    if any(validation_issues.values()):
        ...
        raise ValueError(
            "Selection validation failed: " + "; ".join(formatted_issues)
        )
    ...
    # Advance next_p_number
    if kept_p_ids:
        last_p = int(kept_p_ids[-1][1:])  # Extract number from P-ID
    ...
    trace_writer.write_json("result.json", {
        "outcome": outcome,
