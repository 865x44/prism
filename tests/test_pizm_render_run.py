...
def test_auto_render_accepts_v1_auto_layout(tmp_path):
        ...
        "candidate_assessments": [
            {
                ...
                "disposition": "KEEP",
                "standalone_quality": "strong",
                "unique_residue": "Latency residue",
                "nearest_overlap": None,
                "reason": "Causal mechanism",
            }
        ],
        "bundles": [],
        "next_reasoning_move": "DEEP",
    ...
    dev_sha = (tmp_path / ".ai" / "pizm" / f"run-{run_id_v2}" / "development-v2.sha256").read_text().strip()

    rev = review_payload("P", "P1")
    rev["frozen_hash"] = dev_sha
    freeze_stage(tmp_path, "deep-review-v2", run_id_v2, rev)

    run_dir_v2 = tmp_path / ".ai" / "pizm" / f"run-{run_id_v2}"
    out_v2 = tmp_path / "run_v2.md"
